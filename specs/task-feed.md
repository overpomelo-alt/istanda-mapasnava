...
| Q2 | Drive 命名? | **`中文名_docId`**(例:`林惠卿_2l95Zhad`) |
...
│  ─────────────────────          │
│                                 │
│  🎙 加配音(可選)                 │  ← Q-D Step 0 後決定:嵌入 or 另開層
│                                 │
└─────────────────────────────────┘

#### 上傳進度(Q-C 拍板:逐張 sequential)
...
for (const photo of selectedPhotos) {
  ...
  const fileId = await uploadToAppsScript(compressed);  // 單張 POST、獨立 5MB 預算
...
7. 檢查 `recordings.createdAt` 是否用 serverTimestamp(若用、Task 5 posts.createdAt 對齊;若沒用、Task 5 仍用 serverTimestamp、不動 recordings)
...
6. ❤️💬↗️ 三鍵接到 posts(複用 Task 3-4 邏輯、改寫的 collection)
7. 首頁右下角加 floating 📷 按鈕(發貼文入口、補替 Stories 列)
...
*Status: 等聖瑱師存檔推 GitHub、然後新對話框接手跑 Step 0 第二層 Recon*