...
*Status: ✅ **Task 4 已 ship(2026-05-26)**、DoD 16/16 驗收 PASS*

**Ship 紀錄(5 commits):**
- spec 初版:`5b9e563` Add Task 4 LINE share spec
- spec 對齊 Step 0:`65a86b5` Update Task 4 spec per Step 0 Recon findings
- Step 1:`06ac104` 加 ↗️ 鈕 + MESSAGES + memberData var
- Step 2:`9d9d288` Web Share API + share popover fallback
- Step 3:`5535cca` deep link scroll + highlight + F5 no-repeat

**DoD 驗收**:16/16 全數就位、Step 1-3 實機 PASS、手機 LINE 分享實測成功(URL 含 `#rec={fileId}`)、console 無 error、git 證實三個 Step commit 只動 `member.html`(index.html / style.css / Apps Script 零改動)。
```
